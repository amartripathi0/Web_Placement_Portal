...
import { companyProfileUpdate, getCompanyData } from "../../../../redux/features/company/utilService/companyUtilSlice";
import { RESET } from "../../../../redux/features/company/auth/authSlice";

const JobPosts = () => {

  const dispatch = useDispatch();
  const {isLoggedIn , isLoading , isSuccess , company , message , isProfilePhotoUploaded} = useSelector(state => state.companyUtil)
  ...
  function handleUploaJobDetails(data) {
    ...
    const projectData = { typ: "jobs", value: {...data , status : applicationStatus , qualifications} };
  ...
  >
    <div className="bg-slate-100 py-20 w-full rounded-lg shadow-slate-300 shadow-md flex justify-center items-center">
      ...
      >
        ...
        <div className=" flex flex-wrap px-16 gap-4  justify-between">
          ...
          <div className="flex items-center   justify-between w-[45%]  py-3 px-6 rounded-md  hover:bg-purple-100 ">
            <h2 className="text-xl">Application Status:  <span>{company?.jobs.status} </span></h2> 
              <select name="applicationStatus" id="" onChange={(e) => setApplicationStatus(e.target.value)}>
    ...
    <div className={`flex flex-col gap-10 w-full bg-slate-100 full ${company?.jobs.length !== 0 && "py-20"} rounded-lg shadow-slate-300 shadow-md  justify-center items-center`}>
      {company?.jobs.map((eachJob) => (

        <div key={nanoid()} className={`flex flex-col p-14 px-20 gap-6 bg-white rounded-md w-[85%] shadow-grey-300 shadow-md transition-all duration-150 ${eachJob.status == "Open" && "hover:shadow-green-500"  } ${eachJob.status == "Closed" && "hover:shadow-red-500"  }`}>
         
         <div className="flex gap-10 items-center text-xl font-semibold">

          <div className="">Job No: {company.jobs.indexOf(eachJob) + 1}</div>
          <div className="flex gap-3  items-center justify-center">
              <h1 className="text-xl ">Status: </h1>
              <span className={` ${eachJob.status == "Open" && " text-green-600"} ${eachJob.status == "Closed" && " text-red-500"}`}>{eachJob.status}</span>
          </div>
         </div>
          ...
          <p className="text-xl mt-5 ">Description: <span className="font-medium">{eachJob.description}</span></p>
        </div>
      ))}
    </div>
  </div>
