...
const NavbarDashboard = ({isLoading , userName , heading ,navbarButtonHandler , notficationCount , notficationFrom , notficationTitle , notificationBody ,sidemenuExpanded}) => {
  ...
  return (
    <nav
     ...
     <div className='h-full  flex items-center gap-4 max-sm:hidden'>
        <h1 className='text-[1vw] max-tablet:text-lg font-medium hover:underline hover:font-semibold'>{userName}</h1>
      </div>

      <h1 className='text-[1.5vw] max-tablet:text-xl max-sm:text-base font-medium'>{heading}</h1>
      ...
      <div className='flex justify-between  items-center h-full gap-10 '       >
        ...
        >
              {/*notification count  */}
              <div 
              className='rounded-full bg-red-500 text-white h-4 w-4 absolute -top-5 left-2 flex-center  text-sm p-4 font-medium'>{notficationCount ? notficationCount+'+' : 0}
              </div>
              <FaBell
        ...
        className='flex flex-col fixed  bg-blue-100 gap-4 top-20 p-5 rounded-lg right-10  w-1/5 ' >
          ...
          <div className='p-4 flex flex-col gap-2 bg-pink-50 rounded-2xl hover:bg-pink-100 transition-all  duration-200'>
              <div className='flex justify-between text-xl'>
                  <h1 className='bg-white p-2 rounded-xl hover:bg-slate-50'>Titile: <span className=' font-semibold'>{notficationTitle}</span>  </h1>
                  <h1 className='bg-white p-2 rounded-xl hover:bg-slate-50'>From: <span className=' font-semibold' >{notficationFrom}</span> </h1>
              </div>
              <p className='text-lg font-medium text-justify px-2'>{notificationBody}</p>
          </div>
        </div>}
