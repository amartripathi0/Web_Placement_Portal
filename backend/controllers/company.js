...
const handleCompanySignUP = asyncHandler(async (req, res , next ) => {
    ...
    try {
      ...
      const { emailID, firstName, lastName, phone, password, cpass, staffID } = personalDetail
        
        console.log(req.body); 
  
      if (!emailID || !password || !firstName || !lastName || !phone || !cpass || !staffID || !company) {
        res.status(401);
  ...
  try{
    ...
    throw new Error("Unsufficent data");
        ...
        if(dataType === "personalDetail"){
          updatedData = await Company.updateOne({_id : req.user.id} , {$set : {"personalDetail" : {...personalDetail ,password : user.personalDetail.password , ...dataVal}}})
        }
        else if(dataType === 'jobs'){
          updatedData = await Company.findByIdAndUpdate({_id : req.user.id} , {$push : {"jobs" : dataVal}} , {new : true})
          // if(updatedData) {
  ...
  try{
    if(!req.file){
      ...
      const upload = await uploadToCloudinaryModule.cloudinary.uploader.upload(
        req.file.path,
        { folder: "Placement_Web_Portal/company/profile_photo", resource_type: "image", public_id : "profilePic"
      }
      ); 
      if(upload){
        const personalDetail = req.user.personalDetail;
...
const handleGetStudentDetails = asyncHandler(async (req, res , next) => {  
  try {
    ...
    const studentDetails = await Student.findById(req.params.id).select("-personalDetail.password")
    if(studentDetails){
