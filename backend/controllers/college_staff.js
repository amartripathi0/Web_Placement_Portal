  ...
  try{
    ...
    throw new Error("Unsufficent data");
        ...
        if(dataType === "personalDetail"){
         updatedStaff = await CollegeStaff.updateOne({_id : req.user.id} , {$set : {"personalDetail" : {...personalDetail ,password : user.personalDetail.password , ...dataVal}}})
        }
  ...
  try{
    if(!req.file){
      ...
      const upload = await uploadToCloudinaryModule.cloudinary.uploader.upload(
        req.file.path,
        { folder: "Placement_Web_Portal/college/profile_photo", resource_type: "image", public_id : "profilePic"
      }
      ); 
      if(upload){
        const personalDetail = req.user.personalDetail;
  ...
  try {
    ...
    if(data){
      ...
      if(student){
            ...
            const updateStudent =  await Student.updateOne( {_id : data.id} , {$set : { "personalDetail" : {...data.personalDetail , password : studentPass} , ...data}})
