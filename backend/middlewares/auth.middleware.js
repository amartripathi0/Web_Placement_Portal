        ...
        if (!documentObjOfUser) {
          ...
          if (userType === "student") {
            ...
            user = await CollegeStaff.findById(userId).select(
              ...
              .populate({ path: "companyDetails" , select: "-personalDetail.password" });
           
          } else if (userType === "company") {
            user = await Company.findById(userId).select(
              "-personalDetail.password"
            ).populate({ path: "studentDetails" , select: "-personalDetail.password" });
          }
          if (!user) {
...
const handleResetPassword = (userType) =>  asyncHandler(async (req, res ,next) => {
  try {
  ...
  if (userType === "student") { 
    ...
    if(isValdid){
        const hashedPass = await bcrypt.hash(newPassword , 10)
        const updatedUser = await CollegeStaff.updateOne({_id : user._id } , { $set : {"personalDetail" : {...user.personalDetail , password : hashedPass }}} )
    ...
    if(isValdid){
        const hashedPass = await bcrypt.hash(newPassword , 10)
        const updatedUser = await Company.updateOne({_id : user._id } , { $set : {"personalDetail" : {...user.personalDetail , password : hashedPass }}} )
...
module.exports = {
  isSignedIn,handleResetPassword
};
